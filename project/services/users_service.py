...
class UsersService:
    ...
    def update_password(self, data, refresh_token):
        ...
        if user:
            ...
            self.dao.update(login=user.email, data={"password": generate_password_hash(data.get('password_1'))})
            return self.check(login=user.email, password=data.get('password_1'))
